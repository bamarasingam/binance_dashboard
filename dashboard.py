...
with tab2:
    ...
    y_all_pred_scaled = lr.predict(X_all_scaled)
    ...
    # Add actual prices
    fig.add_trace(go.Scatter(x=df_lr.index, y=df_lr['close'],
                            mode='lines',
                            name='Actual Price'))

    # Add predicted prices
    fig.add_trace(go.Scatter(x=df_lr.index, y=y_all_pred.flatten(),
                            mode='lines',
    ...
    future_y_pred_scaled = lr.predict(future_X_scaled)
    future_y_pred = scaler_y.inverse_transform(future_y_pred_scaled)

    # Add future predictions to the plot
    fig.add_trace(go.Scatter(x=future_dates, y=future_y_pred.flatten(),
                            mode='lines',
                            name='Future Predictions',
                            line=dict(dash='dash')))

    # Update layout
    fig.update_layout(title='Actual, Predicted, and Future Prices',
                    xaxis_title='Date',
                    yaxis_title='Price',
                    legend_title='Legend')
